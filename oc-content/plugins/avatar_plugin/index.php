...
function avatar_upload($userId) {
    //если файл загружен
    if($_POST['avatar_uid']==$userId){
        insertAvatar($userId);
...
function avatar_init() {
    ...
    avatar_form();
    //сохранение аватара
    avatar_upload(osc_logged_user_id());
}
function avatar_form(){ ?>
	<div class="control-group">
        ...
        <div class="controls">
            ...
            <form action="<?=osc_real_url()?>" method="post" enctype="multipart/form-data" >
                <input name="avatar_uid" type="hidden"  value="<?= osc_logged_user_id()?>"/>
                <input id="pAvatar"  name="avatar" type="file" />
            <button type="submit" name="avatar_save" id="avatar_save">сохранить</button>
